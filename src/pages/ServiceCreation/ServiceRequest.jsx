...
function ServiceRequestForm() {
	...
	function CheckLogin() {
		if (firstName && lastName && email) {
			...
			return (
				<div className=" grid-flow-row items-center justify-center">
					<p className="text-red-600 text-1xl ">
						Please login into the system for enable this feature !
					</p>
					<button
						className=" bg-gray-300 text-white font-bold py-2 px-4 rounded"
						onClick={() => {
	...
	function create() {
		axios
			.post("http://localhost:8080/serviceReq", {
				name: name,
		...
		<div classNAme=" transition-all relative justify-center items-center shadow-2xl border-2 border-gray m-5 p-10">
			<div className="border-2 border-gray-200 shadow-md text-center p-10  m-16">
				...
				<textarea
					rows="4"
					cols="50"
					className=" resize-none p-2.5 text-sm text-gray-900 bg-gray-200 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
					id="updatedDesc"
					placeholder="Tell about that service"
					onChange={(e) => {
