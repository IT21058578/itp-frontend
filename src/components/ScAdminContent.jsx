...
const ScAdminContent = ({ service }) => {
	...
	axios
		.delete(`http://localhost:8080/deleteCate/${sid}`)
		...
		<div className="inline-flex transition-all">
			<div className="w-56 h-80 m-2 relative overflow-hidden shadow-lg transition-all rounded-2xl hover:bg-transparent">
				...
				<div className="z-10 bg-gray-800 absolute bottom-0 right-0 left-0 py-6 px-6 hover:opacity-[0.3] hover:bg-transparent">
					<span className="uppercase font-light text-white text-cu tracking-cu2">
						{service.name}
					</span>
					<p className="mt-1 text-orange-200">{service.description}</p>
				</div>
			</div>

			<div className=" relative overflow-hidden transition-all top-36">
				<button
					class="m-2 inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-md"
					onClick={() => {
						setSid(service.id);
						DeleteAlert();
					}}
